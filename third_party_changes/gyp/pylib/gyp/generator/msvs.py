...
def _GetPathOfProject(qualified_target, spec, options, msvs_version):
  ...
  if not proj_filename:
    proj_filename = (spec['target_name'] + options.suffix +
                     msvs_version.ProjectExtension(spec['type']))
...
def _GetMSBuildWixGlobalProperties(spec, guid, gyp_file_name):
  properties = []
  ...
  properties.append(
    ['PropertyGroup', {'Label': 'Globals'},
      ...
      ['ProjectGuid', guid],
      ['SchemaVersion', '2.0'],
      ['OutputName', spec['target_name']],
      ['OutputType', 'Package'],
      ['WixTargetsPath', {'Condition': r" '$(WixTargetsPath)' == '' AND '$(MSBuildExtensionsPath32)' != '' "}, r'$(MSBuildExtensionsPath32)\Microsoft\WiX\v3.x\Wix.targets'],
      ['WixTargetsPath', {'Condition': r" '$(WixTargetsPath)' == '' "}, r'$(MSBuildExtensionsPath)\Microsoft\WiX\v3.x\Wix.targets']])
  
  for (name, settings) in sorted(spec['configurations'].iteritems()):
    configuration, platform = _GetConfigurationAndPlatform(name, settings)
    
    innerProp = ['PropertyGroup', {'Condition': condition},
      ['OutputPath', settings['msvs_configuration_attributes']['OutputDirectory']],
      ['IntermediateOutputPath', settings['msvs_configuration_attributes']['IntermediateDirectory']],
      ['LinkerAdditionalOptions', '-ext WixUIExtension']]

    if configuration == 'Debug':
      innerProp.append(['DefineConstants', 'Debug'])

    properties.append(innerProp)
...
def _GetMSBuildWixProjectReferences(project, options):
  references = []
  if project.dependencies:
    group = ['ItemGroup']
    for dependency in project.dependencies:
      ...
      project_ref = ['ProjectReference',
        {'Include': relative_path},
        ['Name', dependency.name + options.suffix],
        ['Project', guid],
        ['Private', 'True'],
        ['DoNotHarvest', 'True'],
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
  ...
  import_wix_targets_section = [
      ['Import', {'Project': r'$(WixTargetsPath)'}]]
  ...
  if spec['type'] == 'wix_installer':
    ...
    content += _GetMSBuildWixGlobalProperties(spec, project.guid, project_file_name)

    extension_to_rule_name['.wxs'] = 'Compile'

    content += _GetMSBuildSources(
      spec, sources, exclusions, rule_dependencies, extension_to_rule_name,
      actions_spec, sources_handled_by_action, list_excluded)
    content += _GetMSBuildWixProjectReferences(project, options)
    content += import_wix_targets_section
  else:
    #normal file
    content = [
      'Project',
